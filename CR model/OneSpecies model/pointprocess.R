...
setwd("C:/Users/alexd/Dropbox/R Folder/PhD/Spatial/CR Model/OneSpecies model")
sourceCpp("code.cpp")
...
# simulation soft
{
  list_traps <- simulate_softcore_cpp(theta = .05, beta = 1000, niter = 1000, 
...
for(chain in 1:nchain) {
  ...
  while(iter < niter){
    ...
    if(iter %% iterForLambda == 0){
      ...
      lambda_newPoints <- lambda_newPoints + (meanMHratios - .2) * w * (1 / floor(iter / iterForLambda))
      mh_ratios <- rep(NA, iterForLambda)
    }
    ...
    # } else {
      Sigma_proposal <- diag(c(epsilon_beta, epsilon_logtheta), nrow = 2)
    # }
    
    proposed_theta <- mvrnorm(1, c(beta, log(theta)),
                              Sigma = Sigma_proposal)
    beta_star <- proposed_theta[1]
    theta_star <- exp(proposed_theta[2])
    ...
    params_values[iter,c(1,2)] <- c(beta, log(theta))
