...
update_p <- function(p0, sigma, s, CH, trapsX, trapsY, 
  ...
  if(p0_star > 0 & p0_star < 1){
    ...
    loglikelihood_p0_star <- loglikelihood_p_cpp(p0_star, sigma, CH, s,
                                                 trapsX, trapsY, S_usage, N)
    
    loglikelihood_p0 <- loglikelihood_p_cpp(p0, sigma, CH, s,
                                            trapsX, trapsY, S_usage, N)
    
    logprior_p0_star <- dbeta(p0_star, a_p, b_p, log = T)
  ...
  sigma_star <- rnorm(1, sigma, sd = sigma_sigma_prop)
  
  if(sigma_star > 0){
    
    loglikelihood_sigma_star <- loglikelihood_p_cpp(p0, sigma_star, CH, s,
                                                    trapsX, trapsY, S_usage, N)
    
    loglikelihood_sigma <- loglikelihood_p_cpp(p0, sigma, CH, s,
                                               trapsX, trapsY, S_usage, N)
    
    logprior_sigma_star <- dnorm(sigma_star, sigma_0, sd_sigma, log = T)
    
    logprior_sigma <- dnorm(sigma, sigma_0, sd_sigma, log = T)
...
update_z <- function(z, s, beta, N, psi){
  ...
  for (j in (D+1):M) {
    ...
    prob_numerator <- exp(loglikelihood_xi_uncaptured(s[j,], K, S_usage, trapsX, trapsY, p0, sigma))
    prob_denominator <- 1#(1 - exp(loglikelihood_xi_uncaptured(s[j,], K, S_usage, trapsX, trapsY, p0, sigma)))
    
    R <- prob_numerator / prob_denominator
    
    mh_ratio <- phi * R / (1 - phi + phi * R)
...
log_prior <- function(theta1, theta2, theta12, beta1, beta2,
                      a_theta, b_theta, a_beta, b_beta){
  
  dgamma(theta1, a_theta, b_theta, log = T) + dgamma(theta2, a_theta, b_theta, log = T) + 
    dgamma(theta12, a_theta, b_theta, log = T) + dgamma(beta1, a_beta, b_beta, log = T) + 
    dgamma(beta2, a_beta, b_beta, log = T)
...
hastings_ratio <- function(data1, data2, N1, N2, 
  ...
  den <- log_f_bivsoftcore_cpp(data1, data2, N1, N2, beta1, beta2, theta1, theta2, theta12) + 
    log_prior(theta1, theta2, theta12, beta1, beta2, a_theta, b_theta, a_beta, b_beta)
  ...
  num <- log_f_bivsoftcore_cpp(data1, data2, N1, N2, beta1_star, beta2_star, theta1_star, theta2_star, theta12_star) + 
    log_prior(theta1_star, theta2_star, theta12_star, beta1_star, beta2_star, a_theta, b_theta, a_beta, b_beta)
...
update_x_all_foreach <- function(x_all, N_all, niter,
                                 theta, beta, Sigma_prop, Sigma_newpoint, 
                                 allTraps, R_traps,
                                 a1, b1, a2, b2){
